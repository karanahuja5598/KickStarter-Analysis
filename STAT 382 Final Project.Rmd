...
# Read 4 of same data set, combine.
# read = list.files(pattern="*.csv")
# datatest<-lapply(read,read.csv,na.strings=c("","NA"))
#data1<-data.frame(datatest[1])
#data2<-data.frame(datatest[2])
#data3<-data.frame(datatest[3])
#data4<-data.frame(datatest[4])
#data<-rbind(data1,data2,data3,data4)
#write.csv(data,file = "KickstarterBig.csv")
...
# TODO : make the success / fail samples and tables for prelim observations.
Succ<-dataFinal[dataFinal$state == "successful",]
summary(Succ)
Fail<-dataFinal[dataFinal$state == "failed",]

# need to add a % of goal reached.
dataFinal$pReached<- ifelse(dataFinal$state == "successful",((dataFinal$pledged/dataFinal$goal)*100),
                            ((dataFinal$goal - dataFinal$pledged) / dataFinal$goal)* 100)
...
plot(dataFinal$usd_pledged, ylab = "Amount Pledged in USD", main = "USD Pledged", col = "goldenrod1")
...
#USD Pledged
hist(dataFinal$usd_pledged, xlab = "Amount Pledged in USD", main = "USD Pledged", col = "goldenrod1")
boxplot(dataFinal$usd_pledged, ylab = "Amount Pledged in USD", main = "USD Pledged", col = "goldenrod1")
qqnorm(dataFinal$usd_pledged, ylab = "Amount Pledged in USD", main = "USD Pledged", col = "goldenrod1")
...
plot(dataFinal$id, dataFinal$backers_count, xlab = "Category", ylab = "Backer Count", main = "Category vs Backer Count", col = "deepskyblue3")

plot(dataFinal$id, dataFinal$goal, xlab = "Category", ylab = "Goal", main = "Category vs Goal", col = "purple3")

plot(dataFinal$id, dataFinal$pledged, xlab = "Category", ylab = "Pledged", main = "Category vs Pledged", col = "indianred3")

plot(dataFinal$id, dataFinal$usd_pledged, xlab = "Category", ylab = "USD Pledged", main = "Category vs USD Pledged", col = "goldenrod1")

```

```{r scatter plot matrix}

pairs(~dataFinal$backers_count + dataFinal$goal + dataFinal$pledged + dataFinal$usd_pledged +
       dataFinal$id, main = "Backer Count, Goal, Pledged, USD Pledged vs Category")

```

```{r correlation}

cor(dataFinal$id, dataFinal$backers_count)

cor(dataFinal$id, dataFinal$goal)

cor(dataFinal$id, dataFinal$pledged)

cor(dataFinal$id, dataFinal$usd_pledged)

cat("All of our correlations are negative. This suggests that there is an inverse relationship between category and backer count, goal, pledged, and usd_pledged.")
